	...
	return (
		...
		>
			...
			<section className="row-span-7 h-full max-h-full overflow-y-auto py-3 flex gap-4 flex-col">
				{!current?.cart && <span className="text-sm">Giỏ hàng đang trống</span>}
				{current?.cart &&
					current?.cart?.map((el) => (
						<div key={el._id} className="flex border-b border-gray-500 pb-8 justify-between items-center">
							<div className="flex gap-2">
								<img src={el.thumbnail} alt="Ảnh sản phẩm" className="w-[70px] h-[70px] object-cover" />
								<div className="flex flex-col gap-2">
									<span className="font-medium text-sm text-main">{el.title}</span>
			...
			<div className="row-span-2 h-full flex flex-col justify-between gap-4">
				<div className="flex items-center gap-2 justify-between pt-4 border-t border-gray-500">
					<span>Tổng cộng:</span>
					<span>{`${formatMoney(formatPrice(current?.cart.reduce((sum, el) => sum + Number(el.price), 0)))} VND`}</span>
				</div>
